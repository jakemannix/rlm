...
def main() -> None:
    ...
    ap.add_argument("--use-match-gate", action="store_true",
                    help="feed the read gate a qᵀM₂q retrieval-confidence signal (auto-associative "
                         "match store) instead of read magnitude — the principled abstention lever (review C3)")
