...
def main() -> None:
    ...
    for _ in range(args.facts):
        f = gen.make_fact()
        ids = torch.tensor(
            [[tok.bos_token_id] + tok(f.verbatim_prompt, add_special_tokens=False)["input_ids"]],
            device=args.device,
        )
        ...
        row = {
            "prompt": f.verbatim_prompt,
            "answer": f.answer,
            "base_gold_logprob": float(base_logp[gold]),
            "base_gold_rank": int((base_logp > base_logp[gold]).sum()),
            "sweep": {},
        }
        for lam in LAMBDAS:
    ...
    for lam in LAMBDAS:
        ...
        s = summary[lam]
        print(
            f"{lam:>6} {s['mean_gold_lift_nats']:>18.2f} {s['top5_frac']:>10.2f} "
    ...
    if not passing:
        print(
            "→ post-norm injection cannot steer this base; branch to the two-pass soft-token arm."
        )
